...
class ConfusionMatrix:
    ...
    def all_metrics(self):
        return get_mious(self.tp, self.union, self.count)

    def all_metrics_with_f1(self):
        return get_seg_metrics(self.tp, self.union, self.count)


def get_mious(tp, union, count):
    miou, macc, over_all_acc, iou_per_cls, acc_per_cls, _, _ = get_seg_metrics(tp, union, count)
    return miou, macc, over_all_acc, iou_per_cls, acc_per_cls


def get_seg_metrics(tp, union, count):
    ...
    acc_per_cls = tp / count.clamp(min=1) * 100
    # F1 = 2TP / (2TP + FP + FN), and union = TP + FP + FN.
    f1_per_cls = 2 * tp / (tp + union).clamp(min=1) * 100
    over_all_acc = tp.sum() / count.sum().clamp(min=1) * 100

    miou = torch.mean(iou_per_cls)
    macc = torch.mean(acc_per_cls)  # class accuracy
    mf1 = torch.mean(f1_per_cls)
    return (
        miou.item(),
        macc.item(),
        over_all_acc.item(),
