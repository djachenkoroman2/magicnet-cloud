...
class _FurthestPointSamplingCUDA(Function):
    @staticmethod
...
furthest_point_sample = _FurthestPointSamplingCUDA.apply
...
class _GatherOperationCUDA(Function):
    ...
    def backward(ctx, grad_out):
        ...
        grad_features = torch.zeros(
            [B, C, N], dtype=grad_out.dtype, device=grad_out.device, requires_grad=True)
...
def _torch_gather_operation(features: torch.Tensor, idx: torch.Tensor) -> torch.Tensor:
