...
def _has_pointnet2_cuda(tensor: torch.Tensor = None) -> bool:
...
class _GroupingOperationCUDA(Function):
    ...
    def forward(ctx, features: torch.Tensor, idx: torch.Tensor) -> torch.Tensor:
        ...
        _, C, N = features.size()
        output = torch.empty((B, C, nfeatures, nsample), dtype=features.dtype, device=features.device)
    ...
    def backward(ctx, grad_out: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        grad_features = torch.zeros([B, C, N], dtype=grad_out.dtype, device=grad_out.device, requires_grad=True)
...
def grouping_operation(features, idx):
    if not _has_pointnet2_cuda(features):
        return torch_grouping_operation(features, idx)
    return _GroupingOperationCUDA.apply(features, idx)


class _GatherOperationCUDA(Function):
...
def torch_gather_operation(features, idx):
    all_idx = idx.unsqueeze(1).expand(-1, features.shape[1], -1).long()
    return features.gather(2, all_idx)


def gather_operation(features, idx):
    if not _has_pointnet2_cuda(features):
        return torch_gather_operation(features, idx)
    return _GatherOperationCUDA.apply(features, idx)


def _ball_query_fallback(radius: float, nsample: int, xyz: torch.Tensor, new_xyz: torch.Tensor) -> torch.Tensor:
    dist = torch.cdist(new_xyz, xyz)
    within_radius = dist <= radius
    masked_dist = dist.masked_fill(~within_radius, torch.finfo(dist.dtype).max)
    idx = masked_dist.argsort(dim=-1)[..., :nsample]
    valid = within_radius.gather(-1, idx)
    nearest_idx = dist.argmin(dim=-1, keepdim=True).expand(-1, -1, nsample)
    first_valid = torch.where(valid, idx, nearest_idx)[..., :1].expand(-1, -1, nsample)
    idx = torch.where(valid, idx, first_valid)
    return idx.int()


class _BallQueryCUDA(Function):
    @staticmethod
...
def ball_query(radius: float, nsample: int, xyz: torch.Tensor, new_xyz: torch.Tensor) -> torch.Tensor:
    if not _has_pointnet2_cuda(xyz):
        return _ball_query_fallback(radius, nsample, xyz, new_xyz)
    return _BallQueryCUDA.apply(radius, nsample, xyz, new_xyz)
